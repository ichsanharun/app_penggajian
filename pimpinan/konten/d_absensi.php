...
$id = $_SESSION['id'];
$queryabsen = "SELECT * FROM `absensi` INNER JOIN `admin` ON `absensi`.`id_karyawan` = `admin`.`id_admin` WHERE `absensi`.`id_karyawan` = '$id' AND `absensi`.`tanggal_absensi` = '$tanggal_absensi'";
    ...
    <table id="tbl">
        ...
        <tr>
            ...
            <?php for ($i=1; $i <= $jumlahhari; $i++) {
              ...
              $restgl = $tgl."-".$j;
              $querykehadiran = "SELECT * FROM `absensi` INNER JOIN `admin` ON `absensi`.`id_karyawan` = `admin`.`id_admin` WHERE `absensi`.`tanggal_absensi` = '$restgl'";
